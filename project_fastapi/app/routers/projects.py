...
from app.models import UserModel, ProjectMembersModel, RoleProject
from app.services import query_user_by_id, save_project, query_project_join, query_project_by_id, join_project, update_project, del_project
from app.dependencies import allow_admin_only, allow_user_and_admin, get_current_user, verify_project_member
...
def create_project_api(request: Request,
    ...
    new_project = save_project(project_data.model_dump(), user_data.id, db)
...
def get_project_api(request: Request,
    ...
    list_project = query_project_join(user_data.id, db, search)
...
def get_project_api(request: Request, project_data = Depends(verify_project_member)):
...
@project_router.post("/{project_id}/member", response_model=ApiResponse[ProjectMemberData])
